...
public class DataSetMunicipios {
	private Map<String, Set<String>> mMunicipios = new TreeMap<>();
	private List<Municipio> lMunicipios = new ArrayList<Municipio>();
	private Map<String, List<Municipio>> mMunicipiosOrdenados = new TreeMap<>();
	...
	public DataSetMunicipios( String nombreFichero ) throws IOException {
		...
		while (lecturaFic.hasNextLine()) {
			...
			try {
				...
				if(!mMunicipios.containsKey(comunidad)) {
					mMunicipios.put(comunidad, new TreeSet<>());
				}
				mMunicipios.get(comunidad).add(provincia);
				if(!mMunicipiosOrdenados.containsKey(provincia)) {
					mMunicipiosOrdenados.put(provincia, new ArrayList<>());
				}
				mMunicipiosOrdenados.get(provincia).add(new Municipio( codigo, nombre, habitantes, provincia, comunidad ));
	...
	public Map<String, List<Municipio>> getmMunicipiosOrdenados() {
		return mMunicipiosOrdenados;
	}
	...
	public Map<String, Set<String>> getmMunicipios() {
		return mMunicipios;
	}
